...
## What the estate paid to learn this
...
## The blind spot, characterised

A sweep is reliably good at turning narration into a standing instruction, and
reliably blind to **a sentence that is itself an instruction to verify, or that
states a limit of the thing being described.**

Both shapes read as hedging. Both are load-bearing. wrench is full of them:

    Regenerate it rather than trusting this paragraph
    Read result.yaml, never the exit status
    Check which bolt you ran before quoting what bolt does
    A fixture set proves agreement only over the values it holds
...
Substituting punctuation is a punctuation change. If a sentence lost more words
than the mark itself, something else went with it. Put the old and the new side
by side and ask what the old one said that the new one does not, rather than
reading the new one and finding it good.

And have somebody else validate. A writer has just made a hundred judgment calls
and is the worst possible reader of them.
