...
somebody naming them instead.
    ...
    validator   sized against WhiskeyTuesday/zig-jsonschema, 2 stars, carried
                over from earlier notes and never compared against
...
question is asked, the answer is here and not in another GitHub search sorted
by stars.
...
A carried-over choice is not a choice. The validator arrived from earlier notes
and was treated as settled. Nothing marked it as unexamined,
because a note that names one option looks the same as a note that compared
several.

## What to do instead

Survey before sizing. It costs two commands.
...
Name the remote and the commit in any claim about a library, because the names
collide and a bare name is ambiguous.
