...
number that agrees with it gets published.** The script did not merely break; it
broke in the direction nobody checks.
...
Measured against the real probe: the import exits 1 run directly and 0 through
the pipe. The evidence in that finding is correct only because the traceback
prints itself. Had the import started succeeding, the section would have gone
quiet and no fallback would have fired, and silence reads as nothing to report,
not as an expectation violated.

`set -eu` does not cover this. It does not apply inside a pipeline.

The second script's guard was dead for a different reason, and the two need
separating because they need different fixes. It ended
...
Assert that the artifact exists before measuring it. A missing binary should end
the script, not produce a datum.
...
Write the unexpected branch loudly. Every probe of an expected failure needs a
branch that shouts when the expectation is violated. A probe with only the
...
This one is a layer down, in the evidence and not in the check. A gate caught
the earlier case because a gate cannot decide the number looked fine. No gate
runs a `repro.sh`, so nothing catches this except writing the script to fail
...
the measurement instead of reading my output, which is the only reason the wrong
number cost nothing.
