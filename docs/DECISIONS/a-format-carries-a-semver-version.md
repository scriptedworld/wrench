...
existed carries none, and absent means the document claims nothing instead of
...
The pattern is there to reject things that are not semver: `1`, `1.0`,
...
I put it to bolt, which owns what a definitions file means, and bolt said no for
better reasons than I had for asking.

definitions is the one shipped schema whose keys are entirely a user namespace.
The envelope, jig and manifest all have named properties, so `version` sits
beside them and a reader tells metadata from content by looking. In definitions
every key is a placeholder name, so `version` would be both at once, and nothing
in the document would say which was meant.

`{version}` is an ordinary placeholder, not a contrived one: a jig passing a
version to a tool is the common case, and reserving the name costs that.

The FR-4.19 precedent does not carry over, and I was wrong to lean on it. FR-4.19
...
claiming `2.0.0` is refused by the v1 schema instead of being silently checked
against the wrong shape.

Retiring a version deletes its file, and its `$id` is never reused. That is the
same rule `docs/REQUIREMENTS/` applies to requirement ids, for the same reason:
...
The file grows without bound and nothing can be deleted. A retired version stays
in the document because removing a branch changes the meaning of every other
...
actually needed, and this file is here so that decision is executed and not
re-litigated.
