...
## Why this is narrower than a general caution
...
So what decides it is whether somebody is on the other end, whatever the size of
the change.
...
bolt migrated anyway, and its reason stands beside this one: a guard proving two
spellings agree guards against a situation better avoided, and one spelling
needs no guard. Both are right. wrench
...
State it in the change instead of working around it silently. The additive form
