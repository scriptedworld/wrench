...
No consumer's convenience settles what the contract is. That is why the contract
is specified before a pack is written, and why the schemas are one copy and not
one per language.
...
Two commits made from bolt changed wrench's shipped schemas, landing Go-only and
leaving the Python pack broken on `main`. That is precisely the failure this
decision names: a consumer changing the contract to suit itself, with the change
reaching only the pack that consumer happened to use.

`docs/LESSONS/a-schema-change-lands-green-in-the-pack-that-does-not-test-it.md`
has the measurement.

The repair was a route, not a rule. bolt now sends the *shape* it needs and
wrench lands it in every pack with tests in one commit. That worked the same
...
had since measured on its own build. wrench landed the second wording. A consumer
editing the schema directly would have shipped the first.

It cuts both ways. bolt's rows are the source for what a jig and a manifest must
carry; wrench does not invent those shapes. Owning the contract means being the
place it is written down and held consistent, not being the place it is decided
alone.
...
This replaces FR-1.3. It stated who decides, not a property of wrench, so no test
could discharge it. The Go test that cited it was really testing FR-2.3, that the
signature compels a schema and not the right one, and now cites that alone.
