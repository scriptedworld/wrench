...
1. Same input, same output, in every pack and every codec. This is the property
   wrench exists for; a divergence is a defect, not a preference.
...
3. Refusing to buy parity is the worst outcome available. Limiting, filtering or
   warning a caller off a value *because wrench chose not to support it for
   parity* is what this decision forbids. A caller meets a restriction that
   exists for the library's convenience and has nothing to do with their data.

## What it is not

It does not forbid every refusal. Two kinds survive, and the difference is where
...
Control characters. The three options were escape, refuse, or leave it
per-format. Refusing is out by rule 3 and per-format is out by rule 1, so the
answer is escape, and it is also rule 2's good case: Go's YAML emitter is already
correct across all 70 code points measured, and the other two packs already read
what it writes. The work is teaching two hand-written emitters a table that can
be read off a third.

Integers past int64. Widen to float instead of refusing, which is the answer
already given and which this rule confirms.

This is also the one place the rule bites back. Widening an integer is lossy:
Python round trips 2^64 exactly today and would stop. So the packs agree by all
being equally wrong, not by all being right, which is worse than the
control-character case where everyone ends up correct.
...
numeric type, which is FR-2.9's decoded-value contract and no implementation
detail. So the lossy answer stands until somebody wants to pay for the other one,
and this paragraph is here so the next reader knows it was priced and not missed.
...
- Enumerate the value space by type, not by imagination, per
...
- Where the format genuinely cannot hold the value, refuse it and say so in a
  requirement, so the limit is written down instead of discovered.
