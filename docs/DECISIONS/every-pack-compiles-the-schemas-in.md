...
from the directory, not written by hand.
...
## Why generated and not `//go:embed`
...
scoping was the only thing blocking a move of the Go pack under `go/`.
...
Two committed second copies of every schema, which invites the drift FR-3.2
forbids.
...
breaking the tree that way, not by reading the code. Rust has no such
...
watching it, which is worse than reading from disk. The trade is acceptable only
