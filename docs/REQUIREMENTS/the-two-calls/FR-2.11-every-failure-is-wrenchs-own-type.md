...
The family is one catch, and that is what gives the requirement its teeth.
`wrench.Error` in Go and Python, `wrench::Error` in Rust, one name in all three.
Without it, Go's argument checks were plain `errors.New` sentinels and Python's
were bare `ValueError`, so a consumer catching "any wrench failure" missed
exactly the failures that mean it called wrench wrong. Go's sentinels stay
sentinels, so `errors.Is` keeps working, and carry `Step` so `errors.As` reaches
them too.

`schema` against `validate` is the distinction most easily lost. A schema that
will not compile, or whose `$ref` will not resolve, is a fault in the schema:
nothing can be checked against it, and the fix is to the schema. A document
failing a schema that is fine is a fault in the document. Before this row both
were `ValueError` in Python and both were the validator's own type in Go, so a
...
The cause is always reachable, through Python `__cause__`, Go `errors.Unwrap`
and Rust `source()`. A wrap that discarded it would be worse than the leak it
...
**No kind inherits from a language's built-in error**, which looks wrong until
it is measured. Python's `ValueError` is "inappropriate argument value (of
correct type)", `validate` means the value does not match its schema, and
Python's own `json.JSONDecodeError` is a `ValueError`. The inference is obvious
and it is false. Against `{"count": {"type": "integer", "minimum": 0}}`:
...
So wrench's six kinds are orthogonal to the value-against-type split a language
draws, and a consumer catches the family or the kind. This also explains why
`except ValueError` stopped working when the wraps landed, beyond recording that
it did: it was never a statement about wrench's contract, only about what one
bound library happened to raise.
