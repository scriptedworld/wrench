...
## Why TOML does not ship

TOML shipped as a third codec until 2026-09-07. Difficulty had nothing to do
with retiring it: no maintained library in any of the four languages can emit
wrench's canonical TOML, and the hand-written emitters that existed instead were
wrong.
...
Running it in all three packs printed `round trip identity: false` in each. The
cause is the same line in each, recursion into the entry with an empty path:
...
Reaching canonical TOML from a library needs a tree-walking emitter, which is
the thing being retired: BurntSushi's escape replacer and toml_edit's repr
