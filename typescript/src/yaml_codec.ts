 ...
 * sufficient. That comparison is measured.
 *
...
//
// `ScalarTagDefinition<number>` is the library's own name for a normalised
// scalar tag, so the spread has to carry every field the schema reads and a
// replaced hook has to keep the signature the composer calls it with. The
// return type is the one that earns it: `resolve` answers a number OR the
// library's `NOT_RESOLVED`, and a wrapper that dropped the second would turn
// every plain scalar this tag does not match into a string with nothing
// saying so.
const INT_OUT: ScalarTagDefinition<number> = {
  ...intCoreTag,
 ...
 */
const FLOAT_OUT: ScalarTagDefinition<number> = {
  ...floatCoreTag,
...
const NULL_OUT: ScalarTagDefinition<null> = {
  ...nullCoreTag,
  represent: () => "null",
};

/**
 * The CORE tag list with three entries swapped, matched on the tag name.
 *
 * `TagDefinition` is the union of the three tag shapes the schema accepts, and
 * a tag name is the one field all three carry, so the match needs no narrowing
 * and a replacement that is the wrong kind of tag is refused where it is
 * written.
 */
function replaced(tags: readonly TagDefinition[]): TagDefinition[] {
  return tags.map((tag) => {
...
const READ_SCHEMA = new Schema(
  replaced(CORE_SCHEMA.tags).map((tag) =>
 ...
 *
 * `Document` and the node kinds are the library's, so the visitor is narrowed
 * by `kind` rather than asserted: an alias node carries no `tag` and no
 * `items`, and reaching for either is refused here instead of returning
 * `undefined` into a comparison at run time.
 */
function adapt(documents: Document[]): void {
  visit(documents, (node) => {
    if (node.kind === "scalar" && node.tag === STR_TAG) {
      node.style = SCALAR_STYLE.DOUBLE_QUOTED;
    }
    if (node.kind !== "mapping") return;
    node.items.sort((one, two) => {
 ...
 *
 * A key is quoted only where the quoted text is what the file holds, which is
 * why the number and the boolean are spelled and everything else is described.
 * The reading schema is CORE plus `realMapTag`, so a key off a document is a
 * string, a number, a boolean, a null, a sequence or a mapping and nothing
 * else; the last branch is what a value from outside that set is called rather
 * than a shape it is asserted into.
 */
