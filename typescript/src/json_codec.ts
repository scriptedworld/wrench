 ...
 *
 * Declared `this: void`, because it is taken off `JSON` once and called on its
 * own for the rest of the file. That is not a relabelling: the built-in reads
 * no receiver, and writing the type as a method would say the opposite while
 * the code kept doing this.
 */
const rawJSON: (text: string) => unknown = (
  JSON as unknown as {
    rawJSON: (this: void, text: string) => unknown;
  }
).rawJSON;
