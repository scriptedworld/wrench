...
// `ajv/dist/2020.js` and not `ajv/2020`. ajv 8 declares no `exports` map, so
// Node resolves a subpath as a literal file under the package and the short
// spelling is a bare directory that does not exist. The short one worked only
// because `deno.json` mapped it by hand, which is a mapping this pack would
// have had to keep in step with the package's own layout.
...
export function compileSchema(
  ...
  //
  // The `true` makes an invalid schema throw, so the answer on the way out is
  // `true` and the catch below is what a caller sees. The declared type also
  // admits a promise, which is what an `$async` metaschema returns; 2020-12 is
  // not one, so anything other than `true` here is the library doing something
  // this does not understand and is refused rather than dropped on the floor.
  let checked: boolean | Promise<unknown>;
  try {
    checked = new Ajv2020({ strict: false }).validateSchema(parsed, true);
  } catch (cause) {
    throw new SchemaError(`${name}: ${(cause as Error).message}`, { cause });
  }
  if (checked !== true) {
    throw new SchemaError(`${name}: the schema could not be checked`);
  }
